...
##################################################################################        
    def openFile(self):      
        ...
        if path_op == "":return
        if Image.open(path_op).mode == 'RGBA': return                
        path = path_op       
        img = Image.open(path)
        ...
        if chk_cht(path):
#           path2 = path.encode('utf8')
#           path3 = unicode(path2,'utf8')            
           try:
        ...
        plt.ion()
        if img.mode[0] == 'I' or img.mode[0] == 'L' or img.mode[0] == 'F' :       
           im = ax.imshow(img_show,interpolation='none',cmap='Greys_r')
        else:
           im = ax.imshow(img_show,interpolation='none')
...
########################################################################
    def plot_cali_profile(self):
        ...
        if img.mode[0] == 'I' or img.mode[0] == 'F' or img.mode[0] == 'L' :
           ...
           pix = np.asarray([pix_area.getdata()]).reshape(2,xsize)              
           ...
           pix = np.asarray([pix_area.getdata()],dtype=float).reshape(2,xsize,3)            
           pix = pix[:,np.argsort(x_wave),:]
        ...
        if chk_psi == True: 
           ...
           if img.mode[0] == 'I'or img.mode[0] == 'F' or img.mode[0] == 'L':
              ...
              pix = np.asarray([pix_area.getdata()]).reshape(255,xsize)
              ...
              pix = np.asarray([pix_area.getdata()]).reshape(255,xsize,3)              
...
#           plt.legend(loc='best')        
           ...
           if img.mode[0] == 'I' or img.mode[0] == 'F' or img.mode[0] == 'L' :
              ...
              pix = np.asarray([pix_area.getdata()],dtype=float).reshape(2,xsize,3)            
              pix = pix[:,np.argsort(data[:,0]),:]
              plt.imshow(256-pix,aspect='equal',extent=(data[:,0].min(),data[:,0].max(), 100, 0),interpolation='bilinear')# bilinear bicubic aspect='auto''nearest''bilinear',aspect='equal',extent=(0, xsize, 1000, 0)             
...
########################################################################  
    def retranslateUi(self, MainWindow): #Python Spectrum Application
        MainWindow.setWindowTitle(QtGui.QApplication.translate("MainWindow", "PySA 2.0  (build 20170915)", None, QtGui.QApplication.UnicodeUTF8))
        ###tab_1
...
支援 .jpg 及.tiff 的彩色(8bits)或單色(8、16bits)影像。PySA目前主要有以下功能:\n')
...
def readme_about(self):
    ...
    self.README_about.append('Version: 2.0  Build: 20170915  GitHub: https://github.com/lcrobert/PySA') 
