...
#           pix_gray3 = np.asarray(img.getdata().convert('L')).reshape(ysize, xsize)                 
        ...
        plt.gcf().canvas.set_window_title("%s Profile(Y=%d to %d)"%(path,int(box[0])+1,int(box[1])))          
        plt.xlim(1,xsize)
        plt.xlabel('x (pixel)')
        ...
        plt.minorticks_on()
        plt.grid(True)       
        im2 = ax2.plot(plot_x,intensity_box,'k-',label='BT.601')
...
#           plt.grid(True) 
           ...
           if img.mode[0] == 'I' or img.mode[0] == 'F' or img.mode[0] == 'L' :
              ...
              pix = np.asarray([pixo.getdata()]).reshape(255,xsize)              
              pix = pix[:,np.argsort(x_wave)]
              plt.imshow(pix,aspect='auto',interpolation='bilinear',cmap='Greys_r')#aspect='auto''nearest''bilinear',aspect='equal',extent=(0, xsize, 1000, 0)                                        
           else:
              pixo = pixo.resize((xsize,255),Image.BILINEAR)#zoom in
              pix = np.asarray([pixo.getdata()]).reshape(255,xsize,3)            
              pix = pix[:,np.argsort(x_wave),:]
              plt.imshow(256-pix,aspect='auto',interpolation='bilinear')#aspect='auto''nearest''bilinear',aspect='equal',extent=(0, xsize, 1000, 0)           
